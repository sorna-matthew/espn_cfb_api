...
for conference in fbs.getConferences():
    ...
    for matchup in matchups:
        if ((matchup.homeTeam.teamId in apIds) or (matchup.awayTeam.teamId in apIds)) and ((matchup.homeTeam.teamId not in includedIds) and (matchup.awayTeam.teamId not in includedIds)):
            top25matchups.append(matchup)
            includedIds.append(matchup.homeTeam.teamId)
            includedIds.append(matchup.awayTeam.teamId)
...
filename = 'AP_TOP25_week_' + str(week_num) + '.png'
